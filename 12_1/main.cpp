/**
  ...
  *
  * Входные данные
  * Первая строка входного файла содержит строку s (1 ≤ |s| ≤ 100 000).
  * Строка состоит из строчных латинских букв.
  *
  * Выходные данные
  * В первой строке выходного файла выведите два натуральных числа n и m, разделенных пробелом — число вершин и ребер в суффиксном дереве соответственно.
...
public:
    ...
    void printOut()
    {
        std::cout << nodes_.size() << ' ' << (nodes_.size() - 1) <<  '\n';
        for (std::size_t i = 1; i < nodes_.size(); ++i)
        {
            Node& n = nodes_[i];
            std::cout << (n.parent_ + 1) << ' ' << (i + 1) << ' ' << (n.left_ + 1) << ' ' << std::min(sequence_.size(), n.right_) << '\n';
        }
    }
};

static const std::size_t MAX_CAPACITY = 100000;
